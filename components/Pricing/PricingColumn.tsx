...
const PricingColumn: React.FC<Props> = ({ tier, highlight }: Props) => {
    ...
    const handleWhatsAppRedirect = () => {
        const phoneNumber = "917016745669"; // Use international format without + or spaces
    ...
    return (
        <div className={clsx("w-full max-w-sm mx-auto bg-white rounded-xl border border-gray-200 lg:max-w-full", { "shadow-lg": highlight })}>
            <div className="p-6 border-b border-gray-200 rounded-t-xl">
                ...
                <button onClick={handleWhatsAppRedirect} className={clsx("w-full py-3 px-4 rounded-full transition-colors", { "bg-primary hover:bg-primary-accent": highlight, "bg-hero-background hover:bg-gray-200": !highlight })}>
                    Get Started
                </button>
            </div>
